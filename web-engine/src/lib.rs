...
impl Engine {
 fn embedded() -> Self {
  let ac=Aircrafts::from_bytes(include_bytes!("../../vendor/am4/assets/aircrafts-v9.bin")).unwrap();
  let ap=Airports::from_bytes(include_bytes!("../../vendor/am4/assets/airports-v2.bin")).unwrap();
  let mut bytes=include_bytes!("../../vendor/am4/assets/demands-v1.00.bin").to_vec();
  bytes.extend(include_bytes!("../../vendor/am4/assets/demands-v1.01.bin"));
  let demand=DemandMatrix::from_bytes(&bytes).unwrap();
  let distances=DistanceMatrix::from_airports(ap.data());
  Self {ac,ap,distances,demand}
 }
...
thread_local! { static ENGINE: Engine = Engine::embedded(); }
#[wasm_bindgen]
pub fn search_json(input: &str) -> Result<String, JsValue> {
 let p:Params=serde_json::from_str(input).map_err(|e|JsValue::from_str(&e.to_string()))?;
 ENGINE.with(|e|search(e,&p)).map(|v|v.to_string()).map_err(|e|JsValue::from_str(&e))
}
